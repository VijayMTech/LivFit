...
import pandas as pd
from Recommendation_Generation import Generator
from random import uniform as rnd
from ImageFinder.ImageFinder import get_images_links as find_image
from streamlit_echarts import st_echarts

st.set_page_config(
    page_title="LivFit Recommendation",
    page_icon="🍏",
    layout="wide",
...
#     .st-cc {
#         background-color: #27d922 !important;
#     }
...
#     background-color: #27d922 !important;
# }
# .st-emotion-cache-1vzeuhh:focus-visible {
#     box-shadow: rgba(250, 250, 250, 0.2) 0px 0px 0px 0.2rem;
# }
#    .st-ed   {
#     background: linear-gradient(to right, #27d922 0%, #27d922 25%, rgba(172, 177, 195, 0.25) 25%, rgba(172, 177, 195, 0.25) 100%);
# }
# .st-ee  {
#     background: linear-gradient(to right, #27d922 0%, #27d922 50%, rgba(172, 177, 195, 0.25) 50%, rgba(172, 177, 195, 0.25) 100%);
# }
# .st-ef  {
#     background: linear-gradient(to right, #27d922 0%, #27d922 75%, rgba(172, 177, 195, 0.25) 75%, rgba(172, 177, 195, 0.25) 100%);
# }
# .st-eg  {
#     background: linear-gradient(to right, #27d922 0%, #27d922 100%, rgba(172, 177, 195, 0.25) 25%, rgba(172, 177, 195, 0.25) 100%);
# }
# 
...
class Display:
    ...
    def display_bmi(self,person):
        ...
        st.markdown(
            """
            Healthy BMI range is : 18.5 kg/m² - 25 kg/m².
            """)   

    def display_calories(self,person):
        st.header('CALORIES CALCULATOR : ')
        maintain_calories=person.calories_calculator()
        st.write('The outcomes present a range of daily calorie estimates that serve as guidance for determining the appropriate calorie intake per day to either maintain, lose, or gain weight according to the desired rate.')
        for plan,weight,loss,col in zip(self.plans,self.weights,self.losses,st.columns(4)):
            with col:
                st.metric(label=plan,value=f'{round(maintain_calories*weight)} Calories Per day',delta=loss,delta_color="inverse")

    def display_recommendation(self,person,recommendations):
        st.header('DIET RECOMMENDATOR')  
        with st.spinner('Best recommendations are generating.....'):
            meals=person.meals_calories_perc
            st.subheader('Recipes Recommended:')
    ...
    def display_meal_choices(self,person,recommendations):    
        st.subheader('Custom Meal composition:')
...
display=Display()
title="<h1 style='text-align: center;'>LivFit Recommendation</h1>"
...
with st.form("recommendation_form"):
    st.write("Select the values and click the Generate")
    age = st.number_input('Age',min_value=2, max_value=120, step=1)
    height = st.number_input('Height(cm)',min_value=50, max_value=300, step=1)
    weight = st.number_input('Weight(kg)',min_value=10, max_value=300, step=1)
    gender = st.radio('Gender',('Male','Female'))
    activity = st.select_slider('Activity/Workout',options=['Little/no exercise', 'Light exercise', 'Moderate exercise (3-5 days/wk)', 'Very active (6-7 days/wk)',
    ...
    number_of_meals=st.slider('Meals/day',min_value=3,max_value=5,step=1,value=3)
...
if generated:
    ...
    with st.spinner('Best recommendations are generated.....'):
...
if st.session_state.generated:
    with st.container():
        ...
        st.success('Perfect Recommendations!', icon="✅")
    with st.container():
